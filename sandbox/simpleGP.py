...
from gp_systemident.regressors import SimpleExactGPR, NStateSimpleExactGPR
...
if __name__ == '__main__':
    ...
    rmse = cross_val_score(SimpleExactGPR(), X, y, groups=groups, cv=GroupKFold()).mean()
    print(f'rmse={rmse:.2f}')

    cv = GroupKFold()
    train, test = next(cv.split(X, y, groups))
    reg = NStateSimpleExactGPR(3, 3)
